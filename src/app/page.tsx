...
function HomeContent() {
  ...
  // ── CALENDAR ─────────────────────────────────────────────────────────────
  if (step === 'calendar') {
    const BOLO_API_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://world.bomed.ai'
    const calendarReturnUrl = encodeURIComponent(`${BOLO_API_URL}?calendar=connected`)
      ...
      <main className="min-h-screen bg-[#f4f6fb] flex flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm mx-auto flex flex-col gap-7">
          ...
          <div className="flex flex-col gap-3">
            <a
              href={`https://api.bolospot.com/connections/google/authorize?token=${boloToken}&returnUrl=${calendarReturnUrl}`}
              className="bm-btn-primary flex items-center justify-center gap-3 no-underline"
            >
          ...
          <p className="text-center text-[11px] text-[#9ca3af] leading-relaxed px-4">
            Your calendar is connected to Bolospot, not stored by BoMed.<br />
